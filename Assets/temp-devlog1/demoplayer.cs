...
public class demoplayer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!isJumping)
        {
            Vector3 xz = new(demo4GM.Main.currentNode.position.x, transform.position.y, demo4GM.Main.currentNode.position.z);
            float distFromNode = Vector3.Distance(transform.position, xz);
            float currSpeed = pVelocity;
            if (willStop && distFromNode < demo4GM.Main.qDistNodes) currSpeed = distFromNode <= 0.1f ? 0 : pVelocity * (distFromNode / demo4GM.Main.qDistNodes);
        ...
        foreach (var action in actions)
        {
            switch (action.eventType)
            {
                ...
                case Util.EventType.jump:
                    ...
                    Vector3 b = demo4GM.Main.currentNode.position;
